...
error messages.  If you replace the original driver with `RmdConcord::html_documentC` (or the identical `patchDVI::html_documentC`), 
...
properly.  We suggest using the `patchDVI` drivers during
...
version of the `backports` package, which will
automatically be installed when you install
`RmdConcord`.
